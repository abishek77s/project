...
export const TimePatterns: React.FC<TimePatternsProps> = ({ stats }) => {
  ...
  const getTimePersona = (hour: number) => {
    if (hour >= 22 || hour < 4) return { title: 'Night Owl 🦉', desc: 'You thrive in the quiet hours of the night!' };
    if (hour >= 4 && hour < 8) return { title: 'Early Bird 🐦', desc: 'Rising with the sun, seizing every moment!' };
    if (hour >= 8 && hour < 12) return { title: 'Morning Person ☀️', desc: 'Making the most of your mornings!' };
    if (hour >= 12 && hour < 17) return { title: 'Afternoon Achiever 🌤️', desc: 'Peak productivity in the daylight hours!' };
    if (hour >= 17 && hour < 22) return { title: 'Evening Explorer 🌅', desc: 'Finding your flow as the day winds down!' };
    return { title: 'Balanced Browser 🎯', desc: 'Your browsing patterns are well-distributed!' };
  };
    ...
    <div className="text-white p-6">
      ...
      >
        <div className="inline-flex items-center gap-2 mb-4">
          {peakHour.hour >= 22 || peakHour.hour < 6 ? <Moon className="w-8 h-8" /> :
           peakHour.hour < 12 ? <Sunrise className="w-8 h-8" /> :
           peakHour.hour < 17 ? <Sun className="w-8 h-8" /> :
