  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-lg">
      ...
      <p className="text-gray-600 mb-8">
        These interesting websites were visited just once. Maybe it's time to rediscover them?
      </p>
        ...
        {gems.map((gem, index) => (
          ...
          >
            <div className="flex items-center justify-between">
              <h4 className="text-lg font-medium text-gray-800">{gem.domain}</h4>
              <ExternalLink className="w-4 h-4 text-indigo-500" />
            </div>
            <p className="text-sm text-gray-500 mt-1">Category: {gem.category}</p>
          </motion.div>
        ))}
      </div>
    </div>
